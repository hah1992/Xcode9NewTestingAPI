...
@interface NewXCTDemoTests : XCTestCase <XCTWaiterDelegate>

@end

@implementation NewXCTDemoTests {
    XCTestExpectation *_exception;
}

- (void)setUp {
    [super setUp];
    // Put setup code here. This method is called before the invocation of each test method in the class.
    _exception = [[XCTestExpectation alloc] initWithDescription:@"downloadexception"];
}
...
- (void)test_asyncDownload_oldAPI {
...
- (void)test_asyncDownload_newAPI_completed {
    ...
    XCTAssert(result == XCTWaiterResultCompleted, @"failed: %d", result);
...
- (void)test_asyncDownload_newAPI_timeout {
    ...
    XCTAssert(result == XCTWaiterResultTimedOut, @"failed: %d", result);
}
...
- (void)test_asyncDownload_newAPI_incorrectOrder {
    ...
    XCTAssert(result == XCTWaiterResultIncorrectOrder, @"failed: %d", result);
}
...
- (void)test_asyncDownload_newAPI_order {
    ...
    [loader downloadWithCompletion:^(BOOL success, NSError *err) {
        ...
        XCTAssert(result2 == XCTWaiterResultInterrupted, @"failed2: %d", result2);
        [donwloadException fulfill];
    }];
